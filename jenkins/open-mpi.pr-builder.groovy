...
def prepare_build(build_name, label, build_arg) {
  return {
    stage("${build_name}") {
      node(label) {
        checkout_code()
// So this part is to make sure that the Autotools specified in the make_dist_tarball file of the
// branch are the ones we use to build OMPI.  The open-mpi-autotools-build.sh script (which is also
// used in the dist tarball building) does some magic to pull results from S3 if already available, and
// to cache between jobs if builders are either not EC2 instances or are reused
...
//  This is the build bit.  The printenv is debugging.
	withEnv(build_env) {
